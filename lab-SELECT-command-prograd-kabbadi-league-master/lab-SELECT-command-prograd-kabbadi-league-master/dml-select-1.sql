...
select * from player where name LIKE 'S%' and country !='south Afirca';
...
select event_no,clock_in_sec from event order by event_no asc;
...
select stadiumname  from venue where stadiumname like 'complex%' order by stadiumname asc;
...
select winner_teamid,score from outcomes where score <= 35 order by id desc;
...
select * from game where game_date between '01-01-20' and '01-03-20' order by id desc; 
